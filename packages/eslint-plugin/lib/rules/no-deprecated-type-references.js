...
exports.noDeprecatedTypeReferencesRule = void 0;
// tslint:disable object-literal-sort-keys
const utils_1 = require("@typescript-eslint/utils");
const createRule_1 = require("./utils/createRule");
const fixList_1 = require("./utils/fixList");
const getProgram_1 = require("./utils/getProgram");
const replaceImportInFile_1 = require("./utils/replaceImportInFile");
/**
...
function getTypeMappings() {
    const deprecatedToNewTypeMapping = {};
    const newTypeToPackageNameMapping = {};
...
exports.noDeprecatedTypeReferencesRule = (0, createRule_1.createRule)({
    name: "no-deprecated-type-references",
    meta: {
        type: "suggestion",
        docs: {
            description: "Reports on usage of deprecated Blueprint types and recommends migrating to their corresponding replacements.",
    ...
    create: context => {
        ...
        function isDeprecatedTypeReference(name) {
            return (deprecatedToNewType.hasOwnProperty(name) &&
                deprecatedImports.some(deprecatedImport => deprecatedImport.type === "symbol" && deprecatedImport.localSymbolName === name));
        }
        function isDeprecatedNamespacedTypeReference(name, property) {
            return (deprecatedToNewType.hasOwnProperty(property) &&
        ...
        return {
            ...
            TSTypeReference: (node) => {
                switch (node.typeName.type) {
                    case utils_1.TSESTree.AST_NODE_TYPES.Identifier:
                            ...
                            context.report({
                                ...
                                fix: fixer => {
                                    ...
                                    if (program !== undefined && !fixedImportNames.includes(deprecatedTypeName)) {
                                        fixes.addFixes((0, replaceImportInFile_1.replaceImportInFile)(program, deprecatedTypeName, newTypeName, newTypeToPackageName[newTypeName])(fixer));
                        ...
                        return;
                    case utils_1.TSESTree.AST_NODE_TYPES.TSQualifiedName:
                        // only deal with one level of chaining in a qualified name, like "BP.DeprecatedInterface"
                        if (node.typeName.left.type === utils_1.TSESTree.AST_NODE_TYPES.Identifier &&
                            ...
                            context.report({
                                ...
                                messageId: "migration",
                                node,
                                fix: fixer => fixer.replaceText(node.typeName.right, newTypeName),
                            });
                        }
                        return;
                }
            ...
            "TSInterfaceHeritage MemberExpression": (node) => {
                if (node.property.type !== utils_1.TSESTree.AST_NODE_TYPES.Identifier ||
                    node.object.type !== utils_1.TSESTree.AST_NODE_TYPES.Identifier) {
                    return;
                }
...
//# sourceMappingURL=no-deprecated-type-references.js.map